...
uploaded_file = st.file_uploader(
    "Upload MT5 Deals / Trading History CSV",
    type=["csv"]
)
...
if uploaded_file:
    ...
    max_trades_per_min = trades_per_min.max()

    avg_holding = df["Holding Seconds"].mean()
    hft_suspect = (
    ...
    only_scalp = st.checkbox("Show only Scalping trades")
    only_hft = st.checkbox("Show only HFT time-band trades")
    only_arb = st.checkbox("Show only Arbitrage-short trades")
